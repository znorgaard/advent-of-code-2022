...
def make_ranges(range_string):
    """Creates a list of ranges from a string of sectors patrolled by elves

    Args:
        range_string: comma separated string of two ranges defined as a start and stop separated by a "-". ex "4-5,9-10"
    Returns:
         A list of the two ranges
    """
    return [
        # stop is not included in range, so add 1
        range(int(start), int(stop) + 1) for [start, stop] in
        [r_string.split("-") for r_string in range_string.split(",")]
    ]


def check_full_overlap(range1, range2):
    """Check if one range fully overlaps the other

    Args:
        range1: a range object
        range2: a range object
    Returns:
        boolean, True if one range is contained by the other
    """
    overlap = set(range1).intersection(range2)
    return len(overlap) == min(len(range1), len(range2))
...
def check_overlap(range1, range2):
    ...
    Args:
        ...
        range2: a range object
    Returns:
        boolean, True if one range overlaps other
    """

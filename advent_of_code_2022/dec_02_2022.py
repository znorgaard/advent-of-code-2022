...
def decoder_1(strategy_guide):
    """Decodes a strategy guide to list of lists containing throws for each round
    Assumes code_1

    Args:
        strategy_guide: string providing the tournament strategy
    Returns:
        list of lists containing decoded throws for each round
        ex. [ [1,1], [1,2], [2,3] ]
    """
    return [
        [code_1[symbol] for symbol in tourney_round.split(" ")] for tourney_round in strategy_guide.strip().split("\n")
    ]


def decoder_2(strategy_guide):
    """Decodes a strategy guide to list of lists containing throws for each round
    Assumes Code_2
    ...
    Returns:
        ...
        ex. [ [1,1], [1,2], [2,3] ]
    """
    round_codes = [tourney_round.split(" ") for tourney_round in strategy_guide.strip().split("\n")]

    return [
        [
            code_2[tourney_round[0]],
            code_2[tourney_round[1]][code_2[tourney_round[0]] - 1]
        ] for tourney_round in round_codes
    ]


def round_score(them, me):
    """Calculate my score for the round
    Args:
        them: integer indicating their throw
        me: integer indicating my throw
    Returns:
        my score for the round as an integer
    """
    # win patterns (them - me)
    # 1 - 2 = -1
    # 2 - 3 = -1
    # 3 - 1 = 2
    # Lose patterns (them - me)
    # 1 - 3 = -2
    # 2 - 1 = 1
    # 3 - 2 = 1
    if them == me:
...
def total_score(decoded_guide):
    """Calculate total score for decoded strategy guide

    Args:
        decoded_guide: list of lists containing decoded throws for each round (output of `decoder`)
    Returns:
        total score for decoded strategy guide
    """
    return sum([round_score(decoded_round[0], decoded_round[1]) for decoded_round in decoded_guide])

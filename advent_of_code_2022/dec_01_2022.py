...
def max_calories(calorie_string):
    """ Calculates the maximum calories carried by a single elf
    Args:
        calorie_string: string of calorie values
    Returns:
        maximum calories carried by a single elf
    """
    return top_n_elves(calorie_string, n=1)


def top_n_elves(calorie_string, n=1):
    """ Calculates the total calories carried by the top n elves

    Args:
        calorie_string: string of calorie values
        n: the number of top contributing elves to total, default = 1
    Returns:
        sum of the top n elves
    """
    elf_calories = calories_by_elf(calorie_string)
    elf_calories.sort(reverse=True)
    return sum(elf_calories[0:n])


def calories_by_elf(calorie_string):
    """ Returns of a list of calories per elf
    ...
    """
    return [sum([int(cal) for cal in elf_list.split("\n")]) for elf_list in calorie_string.strip().split("\n\n")]
